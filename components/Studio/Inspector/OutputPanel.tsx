...
export default function OutputPanel({ settings, onSettingsChange, onResetStudio, exporting, exportMessage, exportFailed, onExport, onExportPrintPackage, onExportEtsyPackage, onExportSocialPackage, onExportMarketingMockups }: OutputPanelProps) {
  ...
  return (
    <>
      ...
      {settings.format === 'eps' && <div className="mt-2"><StudioNotice tone="warning" title="EPS unavailable" message="A trusted PostScript converter is not installed. Choose SVG or PDF for a production-ready vector master." /></div>}
      ...
      {settings.colour.mode === 'cmyk' && <div className="mt-2"><StudioNotice tone="warning" title="CMYK conversion unavailable" message="A licensed ICC transform is not installed. Choose sRGB to export now; the preview remains colour-managed in sRGB." /></div>}
      ...
      <button type="button" disabled={settings.colour.mode !== 'rgb' || exporting} onClick={onExportMarketingMockups} className="mt-2 w-full rounded-lg border border-white/10 bg-white/[.04] px-3 py-3 text-xs font-semibold text-white/70 transition hover:bg-white/[.08] disabled:cursor-not-allowed disabled:opacity-40">Marketing Mockups</button>
      {exportMessage && !exporting && <div className="mt-3"><StudioNotice tone={exportFailed ? 'error' : 'success'} title={exportFailed ? 'Export not completed' : 'Export complete'} message={exportMessage} /></div>}
      {exportMessage && exporting && <p role="status" className="mt-3 text-[10px] leading-4 text-white/45">{exportMessage}</p>}
    </>
  );
}
