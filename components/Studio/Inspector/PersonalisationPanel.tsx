...
export default function PersonalisationPanel({ value, onChange }: PersonalisationPanelProps) {
  ...
  return (
    <>
      ...
      <label className="mt-5 flex cursor-pointer items-center justify-between rounded-lg border border-white/8 bg-white/[0.025] px-3 py-3">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          {fields.map((field) => (
            <label key={field.id} className={field.id === 'occasion' ? 'col-span-2' : ''}>
              <span className="mb-1.5 block text-[9px] uppercase tracking-[0.18em] text-white/35">{field.label}</span>
          ...
          <label className="col-span-2">
            <span className="mb-1.5 block text-[9px] uppercase tracking-[0.18em] text-white/35">Notes</span>
